...
describe('.toContainHTML', () => {
  test('handles positive and negative cases', () => {
    ...
    expect(() => expect(stringChildElement).not.toContainHTML(fakeElement))
      .toThrowError();
    expect(() => expect(svgElement).toContainHTML(stringChildElement))
      .toThrowError();
    expect(() => expect(grandparent).not.toContainHTML(stringChildElement))
      .toThrowError();
    expect(() => expect(parent).not.toContainHTML(stringChildElement))
      .toThrowError();
    expect(() => expect(child).not.toContainHTML(stringChildElement))
      .toThrowError();
    expect(() => expect(child).not.toContainHTML(stringChildElement))
      .toThrowError();
    expect(() => expect(child).not.toContainHTML(stringChildElementSelfClosing))
      .toThrowError();
    expect(() => expect(child).toContainHTML(nonExistantString)).toThrowError();
    expect(() => expect(parent).toContainHTML(nonExistantString))
      .toThrowError();
    expect(() => expect(grandparent).toContainHTML(nonExistantString))
      .toThrowError();
    expect(() => expect(child).toContainHTML(nonExistantElement))
      .toThrowError();
    expect(() => expect(parent).toContainHTML(nonExistantElement))
      .toThrowError();
    expect(() => expect(grandparent).toContainHTML(nonExistantElement))
      .toThrowError();
    expect(() =>
      expect(nonExistantElement).not.toContainHTML(incorrectStringHtml)
    ).toThrowError();
    expect(() => expect(grandparent).not.toContainHTML(incorrectStringHtml))
      .toThrowError();
    expect(() => expect(child).not.toContainHTML(incorrectStringHtml))
      .toThrowError();
    expect(() => expect(parent).not.toContainHTML(incorrectStringHtml))
      .toThrowError();
  });
