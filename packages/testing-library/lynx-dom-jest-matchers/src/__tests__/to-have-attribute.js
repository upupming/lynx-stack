...
test('.toHaveAttribute', () => {
  ...
  expect(() => expect(queryByTestId('ok-button')).not.toHaveAttribute('type'))
    .toThrowError();
  expect(() => expect(queryByTestId('ok-button')).toHaveAttribute('class'))
    .toThrowError();
  expect(() =>
  ...
  expect(() => expect({ thisIsNot: 'an html element' }).not.toHaveAttribute())
    .toThrowError();
