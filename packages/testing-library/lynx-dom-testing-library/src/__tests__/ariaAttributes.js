...
test('`selected` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('textbox', { selected: true }))
...
test('`pressed` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('textbox', { pressed: true }))
    .toThrowErrorMatchingInlineSnapshot(
...
test('`checked` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('textbox', { checked: true }))
    .toThrowErrorMatchingInlineSnapshot(
...
test('`expanded` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('heading', { expanded: true }))
    .toThrowErrorMatchingInlineSnapshot(
...
test('`busy` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('none', { busy: true }))
    .toThrowErrorMatchingInlineSnapshot(
...
`);
  ...
  expect(getAllByRole('gridcell', { selected: true }).map(({ id }) => id))
    .toEqual([
      'selected-gridcell',
    ]);

  expect(getAllByRole('option', { selected: true }).map(({ id }) => id))
    .toEqual([
      'selected-native-option',
      'selected-listbox-option',
    ]);

  expect(getAllByRole('rowheader', { selected: true }).map(({ id }) => id))
    .toEqual(
      ['selected-rowheader', 'selected-native-rowheader'],
    );

  expect(getAllByRole('treeitem', { selected: true }).map(({ id }) => id))
    .toEqual([
      'selected-treeitem',
    ]);
...
test('`level` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('button', { level: 3 }))
    .toThrowErrorMatchingInlineSnapshot(
...
test('`value.now` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('button', { value: { now: 1 } }))
    .toThrowErrorMatchingInlineSnapshot(
...
test('`value.max` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('button', { value: { max: 1 } }))
    .toThrowErrorMatchingInlineSnapshot(
...
test('`value.min` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('button', { value: { min: 1 } }))
    .toThrowErrorMatchingInlineSnapshot(
...
test('`value.text` throws on unsupported roles', () => {
  ...
  expect(() => getByRole('button', { value: { text: 'one' } }))
    .toThrowErrorMatchingInlineSnapshot(
...
test('`value.text: Matcher` matches `aria-valuetext` on widgets', () => {
  ...
  expect(getByRole('spinbutton', { value: { text: 'zero' } }))
    .toBeInTheDocument();
  expect(getAllByRole('spinbutton', { value: { text: /few|many/ } }))
    .toHaveLength(
      2,
    );
});
