...
test('can find any labelable element when label text is inside other elements', () => {
  ...
  nodeTypes.forEach(nodeType => {
    expect(getByLabelText('Test Label', { selector: nodeType }).nodeName)
      .toEqual(
...
test('queryAllByRole returns semantic html elements', () => {
  ...
  expect(queryAllByRole('progressbar', { queryFallbacks: true })).toHaveLength(
    1,
  );
...
test('using jest helpers to assert element states', () => {
  ...
  expect(() => expect(queryByTestId('count-value')).not.toBeTruthy())
    .toThrowError();
  expect(() => expect(queryByTestId('count-value1')).toBeTruthy())
    .toThrowError();
  expect(() => expect(queryByTestId('count-value')).toHaveTextContent('3'))
    .toThrowError();
  expect(() => expect(queryByTestId('count-value')).not.toHaveTextContent('2'))
    .toThrowError();
});
...
test('using jest helpers to check element attributes', () => {
  ...
  expect(() => expect(getByTestId('ok-button')).not.toHaveAttribute('type'))
    .toThrowError();
  expect(() => expect(getByTestId('ok-button')).toHaveAttribute('class'))
    .toThrowError();
  expect(() =>
...
test('using jest helpers to check element class names', () => {
  ...
  expect(() => expect(getByTestId('delete-button')).not.toHaveClass('btn'))
    .toThrowError();
  expect(() =>
    expect(getByTestId('delete-button')).not.toHaveClass('btn-danger')
  ).toThrowError();
  expect(() => expect(getByTestId('delete-button')).not.toHaveClass('extra'))
    .toThrowError();
  expect(() => expect(getByTestId('delete-button')).toHaveClass('xtra'))
    .toThrowError();
  expect(() =>
    expect(getByTestId('delete-button')).not.toHaveClass('btn btn-danger')
  ).toThrowError();
  expect(() => expect(getByTestId('delete-button')).toHaveClass('btn-link'))
    .toThrowError();
  expect(() => expect(getByTestId('cancel-button')).toHaveClass('btn-danger'))
    .toThrowError();
});
