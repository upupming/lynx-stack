...
test('respects ignores', () => {
  ...
  expect(() => screen.queryByText('foo', { ignore: 'my-thing' })).not
    .toThrowError();
});
...
test('does not suggest when using getByRole', () => {
  ...
  expect(() => screen.getByRole('button', { name: /submit/i })).not
    .toThrowError();
});
...
test(`should not suggest if the suggestion would give different results`, () => {
  ...
  expect(() => screen.getAllByTestId('foo', { suggest: false })).not
    .toThrowError();
});
...
test('should not suggest when suggest is turned off for a query', () => {
  ...
  expect(() => screen.getByTestId('foo', { suggest: false })).not
    .toThrowError();
  expect(() => screen.getAllByTestId(/foo/, { suggest: false })).not
    .toThrowError();
});
