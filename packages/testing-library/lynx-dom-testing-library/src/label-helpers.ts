...
function getLabelContent(element: LynxFiberElement): string | null {
  let textContent: string | null;
  if (element.tagName.toLowerCase() === 'label') {
    textContent = getTextContent(element);
  } else {
    textContent = (element as LynxFiberElement).value || element.textContent;
  }
  return textContent;
}

// Based on https://github.com/eps1lon/dom-accessibility-api/pull/352
function getRealLabels(element: LynxFiberElement) {
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- types are not aware of older browsers that don't implement `labels`
  if ((element as LynxFiberElement).labels !== undefined) {
    return (element as LynxFiberElement).labels ?? [];
  }
...
function isLabelable(element: LynxFiberElement) {
  return (
...
function getLabels(
  ...
  { selector = '*' } = {},
): { content: string | null; formControl: LynxFiberElement | null }[] {
  const ariaLabelledBy = element.getAttribute('aria-labelledby');
  const labelsId = ariaLabelledBy ? ariaLabelledBy.split(' ') : [];
  return labelsId.length
    ? labelsId.map(labelId => {
      const labellingElement = container.querySelector<LynxFiberElement>(
        `[id="${labelId}"]`,
      );
    ...
    : Array.from(getRealLabels(element)).map(label => {
      ...
      const labelledFormControl = Array.from(
        label.querySelectorAll<LynxFiberElement>(formControlSelector),
