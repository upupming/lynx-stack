...
function getElementError(message: string | null, container: LynxFiberElement) {
...
function queryAllByAttribute(
  ...
  return Array.from(
    container.querySelectorAll<LynxFiberElement>(`[${attribute}]`),
  ).filter(node =>
...
function makeSingleQuery<Arguments extends unknown[]>(
  allQuery: QueryMethod<Arguments, LynxFiberElement[]>,
  getMultipleError: GetErrorFunction<Arguments>,
) {
  return (container: LynxFiberElement, ...args: Arguments) => {
...
const wrapSingleQueryWithSuggestion =
  ...
  (container: LynxFiberElement, ...args: Arguments) => {
...
// This is at least statically true and can be verified by accepting `QueryMethod<Arguments, LynxFiberElement[]>`
